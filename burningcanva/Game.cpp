...
void Game::judge_gameplay()
{
	...
	else if (sign == 0)
	{
		AccountManage.add_total(currentPlayer);
		HWND hnd = GetHWnd();
		MessageBox(hnd, _T("Try Again"), _T("提示"), MB_OKCANCEL);
	}
	//更新到文件当中
...
void Game::calculate(int size)
{
	...
	//std::fstream l("tmp.txt", std::ios::app);
	//if (size == 4) l << qtotnumber[0] << std::endl;
	//else if (size == 6) l << qtotnumber[1] << std::endl;
	//else if (size == 8) l << qtotnumber[2] << std::endl;
	//else if (size == 12) l << qtotnumber[3] << std::endl;
}
...
void Game::create_input()
{
	...
	while (1)//等待用户输入
	{
		...
		if (MouseHit())
		{
			...
			{
				COLORREF colo;
				if (x >= 0 && x <= 100 && y >= 800 && y <= 850&&checkcreate())//提交
				{
				...
				{
					HWND hnd = GetHWnd();
					MessageBox(hnd, _T("Still need to create!"), _T("提示"), MB_OKCANCEL);
				}
				else if (x >= 200 && x <= 300 && y >= 500 && y <= 550)
				{
					...
					{
						setfillcolor(WHITE);
						fillrectangle((posj - 1) * 30 + 700, (posi + 1) * 30 + 500, posj * 30 + 700, posi * 30 + 500);
					...
					else if(posj==0&&posi!=0)
					{
						...
						fillrectangle((size - 1) * 30 + 700, (posi - 1) * 30 + 500, (size) * 30 + 700, (posi - 1) * 30 + 500);
						posj = size - 1;
					}
				}
				else if (x >= 0 && x <= 100 && select == 0)//选择颜色
				{
...
void Game::login()
{
	...
	while (1)
	{
		...
		for (int i = 0; i < inpwd.length(); i++)
			inpwd[i] ^= pwds[i];
		Player checkplayer(0, inname, inpwd);//用户名唯一标识
		if (AccountManage.contains(checkplayer))
		{
			HWND hnd = GetHWnd();
			nowplayer = inname;
			nowpwd = inpwd;
